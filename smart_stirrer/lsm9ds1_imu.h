...
typedef struct {
	uint8_t agAddress, mAddress, _mAddress, _xgAddress, accel_enabled, accel_scale, 
        ...
        gyro_enableX, gyro_enableY, gyro_enableZ, gyro_latchInterrupt, mag_enabled, mag_scale, 
...
int16_t m_imu_raw[10]; // a(x, y, z), g(x, y, z), m(x, y, z), temp
uint16_t m_imu_data[10]; 
...
void readAccel(lsm9ds1_config_t * lsm_cfg) {
    ...
    m_imu_raw[0] = ((int16_t)buff[1] << 8) | (int16_t)buff[0]; 
    m_imu_raw[1] = ((int16_t)buff[3] << 8) | (int16_t)buff[2]; 
    m_imu_raw[2] = ((int16_t)buff[5] << 8) | (int16_t)buff[4];
    ...
    for (int i = 0; i <= 2; i++) {
        m_imu_data[i] += (m_imu_raw[i] + 32768);
    }
}
...
void readGyro(lsm9ds1_config_t * lsm_cfg) {
    ...
    m_imu_raw[3] = ((int16_t)buff[1] << 8) | (int16_t)buff[0]; 
    m_imu_raw[4] = ((int16_t)buff[3] << 8) | (int16_t)buff[2];
    m_imu_raw[5] = ((int16_t)buff[5] << 8) | (int16_t)buff[4]; 
    if (lsm_cfg->_autoCalc) {
...
void readMag(lsm9ds1_config_t * lsm_cfg) {
    ...
    m_imu_raw[6] = ((int16_t)buff[1] << 8) | (int16_t)buff[0]; 
    m_imu_raw[7] = ((int16_t)buff[3] << 8) | (int16_t)buff[2]; 
    m_imu_raw[8] = ((int16_t)buff[5] << 8) | (int16_t)buff[4];
    if (lsm_cfg->_autoCalc) {
...
void readTemp(lsm9ds1_config_t * lsm_cfg) {
    ...
    m_imu_raw[9] = ((int16_t)temp[1] << 8) | (int16_t)temp[0];
...
void calibrate(lsm9ds1_config_t * lsm_cfg, bool autoCalc/*=true*/) {
    ...
    for (ii = 0; ii < samples[0]; ii++)  {	
        readAccel(lsm_cfg);
        aBiasRawTemp[0] += m_imu_data[0];
        aBiasRawTemp[1] += m_imu_data[1];
        aBiasRawTemp[2] += m_imu_data[2] - (int16_t)(1./lsm_cfg->aRes);
        readGyro(lsm_cfg);
        gBiasRawTemp[0] += m_imu_data[3];
        gBiasRawTemp[1] += m_imu_data[4];
        gBiasRawTemp[2] += m_imu_data[5];
    }  
    for (ii = 0; ii < 3; ii++) {
...
uint8_t magAvailable(lsm9ds1_config_t * lsm_cfg, lsm9ds1_axis_t axis /* = ALL_AXIS */) {
    ret_code_t err_code;
    uint8_t status[1] = { 0 };
...
void calibrateMag(lsm9ds1_config_t * lsm_cfg, bool loadIn /* = true*/ ) {
    ret_code_t err_code; 
